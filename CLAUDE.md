...
DB 구축 + 검색/추천 로직 구현 완료. `pipeline.run(query, driver, llm_client)` 한 함수로 전체 파이프라인 동작.
...
├── coe/            # 단계 B: 슬롯 추출 (slot_extractor.py — Layer 1/2)
├── llm/            # 단계 C: LLM 출구 (exit.py — recommend())
├── dicts/          # KG 노드명 사전 (category_mapping, keyword_categories, station_coords)
├── scripts/        # 손검증 스크립트 (verify_kg_search.py, verify_coe.py)
├── tests/          # 단위 테스트 (coe/, kg/, llm/, test_pipeline.py — 39개)
